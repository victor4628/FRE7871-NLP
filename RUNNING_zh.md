...
本次统一分析样本为 1,536 份报告、91 家公司。2026 年 9 月 7 日 TA 更新后，必须采用 day -1 股价至少 3 美元、公司季度内保留最早 filing、前后至少 60 个收益观测、`[-60,-6]` 与 `[+4,+63]` 窗口，以及完整控制变量和固定效应。固定 2026 年持仓不能解释为各历史时点的全部 ARK 持仓；完整样本 TF-IDF 也属于回顾性分析。SEC 和 Yahoo 数据可能修订，未来重新下载的数值可能变化。

修改解析规则后，需要更新解析缓存版本，或调用 `src.analysis_data.build_text_data(force=True)` 从 HTML 重建，再执行 Notebook。不要只删除一部分文本矩阵文件。

Table 5 在同一 1,536 份样本上比较不含与包含披露前波动率的两种模型；两列都包含规模、平均美元成交量、前期 SPY 超额收益、10-K 指示变量、公司固定效应和日历季度固定效应。Table 6 使用完整控制变量。报告和 Notebook 中的所有窗口及结果均已按 TA 新版要求重算。
