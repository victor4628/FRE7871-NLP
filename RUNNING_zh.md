...
Tables 5 和 6 现按用户要求比较四种模型：无控制变量、仅公司规模、仅披露前波动率、两者都有。四种模型在每个结果变量内使用相同样本；主模型没有其他控制变量或固定效应。公司规模采用市值对数，披露前 63 个交易日收益仅用于计算基准波动率，不再计算累计前期收益或平均换手率。加入公司、日历季度及报告类型固定效应的结果单独列为稳健性检验。
